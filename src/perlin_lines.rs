...
use palette::{Hsv, Srgb, RgbHue, LinSrgb, FromColor};
use ini::Ini;
...
use rand::{thread_rng, Rng, random, RngCore};
use std::io::Write;
...
fn main() {
    ...
    let color = Srgb::new(0.8, 0.2, 0.1);
    ...
    context.set_source_rgb(212. / 255., 245. / 255., 220. / 255.);
    context.paint();
    ...
    */
    ...
    for z in 1..lines {
        ...
        let z = z as f64;
        let mut noise = 0.;
        let num_points = width * 10;
        for x in 0..num_points{
            context.set_line_width(10.);
            let fnpoints = num_points as f64;
            noise += 1. / fnpoints;
            let x = x as f64;
            let x = fwidth * x / fnpoints;
            let y = z * dist + (z * 20. + 10.) * per.get([30. + noise, 0.]);
            // println!("{} {}", x, y);
